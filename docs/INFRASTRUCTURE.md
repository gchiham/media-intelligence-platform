...
|---|---|---|---|
| `CHEPITA-L4-v1.3.0` | **`ami-0333472587024212a`** | g6.xlarge, GPU L4 | Igual que v1.2.0 + los dos fixes del worker que evitan pérdida silenciosa de grabaciones (publica el done ANTES de borrar el job; `DONE_QUEUE_URL` obligatoria). Trae el caché de HuggingFace poblado, así que arranca en ~7 s en vez de ~83 s. **Usar este para relanzar chepita.** La config de arranque es la misma que v1.2.0 (3 workers, `batch_size=12`). |
| `CHEPITA-L4-v1.2.0` | `ami-02c05ccbd6699df41` | g6.xlarge, GPU L4 | Igual que v1.1.0 + `worker_prefetch.py`/`FasterWhisperProvider` con `large-v3-turbo` por defecto (antes `small`) y soporte de `hotwords` (vocabulario hondureño, ver `docs/EFFICIENCY_REVIEW.md` §5). Superado por v1.3.0 — su `worker_prefetch.py` borra el job antes de publicar el done y deja huérfanos en `PROCESANDO` si se cae en medio. Mantener como rollback. |
...
| `v1.3.0` | `ami-0333472587024212a` | 2026-08-18 | Dos fixes en `worker_prefetch.py` que evitaban perdida silenciosa de grabaciones (ver PR #3): **(a)** publica el evento `done` ANTES de borrar el job de la cola -- al reves, un crash entre ambos dejaba la Grabacion en `PROCESANDO` para siempre con el resultado ya en S3 (la clase de huerfanos que obligo al backfill manual del 2026-07-22); con el orden nuevo el peor caso es una retranscripcion que el consumer descarta por `unique(grabacion_id)`. **(b)** `DONE_QUEUE_URL` pasa de opcional a obligatoria: cuando era opcional, lanzar una flota sin esa env var reproducia en silencio el incidente del AMI v1.0.0 (todo se transcribe y sube a S3 pero ninguna Grabacion sale de `PROCESANDO`, sin un solo error visible). Horneado desde una instancia dedicada (no de produccion) permitiendo el reboot, para un filesystem consistente. Conserva el cache de HuggingFace poblado. | Smoke test en la instancia antes de hornear: 3 grabaciones transcritas (`DONE tnh elapsed=30.8s`), 0 errores/warnings en el log, evento publicado a `media-intel-transcription-done`, y transcripcion confirmada en la RDS (22,774 -> 22,777). |

**Para relanzar chepita cuando se necesite:**
```bash
aws ec2 run-instances --image-id ami-0333472587024212a --instance-type g6.xlarge \
